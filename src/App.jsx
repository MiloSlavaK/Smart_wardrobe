...
}[cat] || 'Сложите аккуратно по швам');

const getDefaultWashing = (cat) => ({
...
}[cat] || 'Следуйте инструкции на ярлычке.');

export const App = () => {
  const { items, stats, addItem, toggleItemCompleted, deleteItem, updateReminder, getRecoveryState: getClosetRecovery } = useClosetItems([DEMO_ITEM]);
    ...
    switch (type) {
      ...
      case 'ask_folding': {
        ...
        if (item && window.sendSmartAppResponse) {
          window.sendSmartAppResponse({ type: 'voice_response', text: `Как сложить ${item.name}: ${item.instruction}`, emotion: 'helpful' });
        }
        break;
      }
      case 'speak_washing':
      case 'ask_washing': {
        const item = params.id ? ctx.items.find(i => i.id === params.id) : null;
        if (item && window.sendSmartAppResponse) {
          window.sendSmartAppResponse({ type: 'voice_response', text: `Совет по стирке для ${item.name}: ${item.washing}`, emotion: 'helpful' });
        }
        break;
      }
      default:
  ...
  // 🔥 getState: СТРОГО по документации - только item_selector с number/id/title
  const getStateForAssistant = useCallback(() => ({
    item_selector: {
      items: items.map(({ id, name }, index) => ({
        number: index + 1, id, title: name,
      })),
  ...
  const { assistant, updateState, updateRecoveryState, sendActionValue, sendSmartAppResponse, cancelTts } = useAssistant(
    handleAssistantAction, smartAppContext.current, getRecoveryState
  );

  useEffect(() => {
    if (assistant) {
      assistantRef.current = assistant;
      if (process.env.NODE_ENV === 'development') window.assistant = assistant;
      window.sendSmartAppResponse = sendSmartAppResponse; // для useSpeech
    }
    return () => {
  ...
  const handleAdd = useCallback((itemData) => {
    const newItem = {
      id: generateId(), name: itemData.name || 'Новая вещь', category: itemData.category || 'другое',
      ...
      nextReminder: itemData.nextReminder || '', completed: false,
    };
  ...
  return (
    <TaskList
      ...
      onAdd={handleAdd} onDone={handleDone} onDelete={handleDelete}
      onUpdateReminder={handleUpdateReminder}
      onAskFolding={handleSpeakInstruction} onAskWashing={handleSpeakWashing}
